...
$link  = '<a href="' . $href . '" class="fi-btn-secondary inline-flex items-center gap-2 px-3 py-1.5 bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-md text-sm font-medium text-gray-700 dark:text-gray-200 hover:bg-gray-50"><i class="fa fa-edit fa-margin"></i> ' . htmlspecialchars($field->custom_field_label) . '</a>';
            ...
            <div class="md:col-span-6 md:col-start-4">
               ...
               @include('core::layout.alerts')
                ...
                <div class="fi-field-wrp mb-4">
                    ...
                    <input type="text" class="fi-input w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md dark:bg-gray-700 dark:text-white" name="custom_values_value" id="custom_values_value" required>
                </div>

                <div class="sm:hidden">
                    <div class="fi-field-wrp mb-4">{{ trans('field') }}: {!! $link !!}</div>
                    <div class="fi-field-wrp mb-4 badge px-2 py-1 rounded-md text-xs font-semibold bg-blue-100 dark:bg-blue-900 text-blue-800 dark:text-blue-200">{{ trans('table') }}: {{ trans($table) }}</div>
                    <div class="fi-field-wrp mb-4 badge px-2 py-1 rounded-md text-xs font-semibold bg-blue-100 dark:bg-blue-900 text-blue-800 dark:text-blue-200">{{ trans('position') }}: {{ $position }}</div>
                    <div class="fi-field-wrp mb-4 badge px-2 py-1 rounded-md text-xs font-semibold bg-blue-100 dark:bg-blue-900 text-blue-800 dark:text-blue-200">{{ trans('type') }}: {{ trans($alpha) }}</div>
                </div>
            </div>
        </div>
